...
Python's Protocol (structural subtyping). Any LLM provider can implement
this interface without inheriting from a base class.
...
class Message:
    ...
    Attributes:
        ...
        images: Optional base64-encoded JPEG images attached to the message
            (for vision models). Each entry is the raw base64 string (no
            "data:" prefix). Providers translate these into their own
            multimodal content format.

    Example:
        system_msg = Message(role="system", content="You are helpful.")
        user_msg = Message(role="user", content="Hello!")
        vision_msg = Message(role="user", content="What's here?", images=(b64,))
    """
...
class CompletionResponse:
    ...
    Attributes:
        ...
        tokens_used: Total tokens consumed (prompt + completion).
            May be 0 if streaming was used or provider doesn't report usage.
...
class LLMProvider(Protocol):
    ...
    This uses structural subtyping (duck typing) via Python's Protocol,
    ...
    Example implementation:
        class CustomProvider:
            def __init__(self, api_key: str):
                ...
                self._default_model = "custom-model"
